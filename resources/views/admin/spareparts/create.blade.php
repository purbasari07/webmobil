...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h2 class="text-3xl font-bold text-slate-800 tracking-tight">Tambah Sparepart</h2>
        <p class="text-slate-500 text-sm mt-1">Buat data master suku cadang baru.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.spareparts.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="brand" id="brand" value="{{ old('brand') }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition text-sm" placeholder="Contoh: Shell / Toyota Genuine Parts">
                @error('brand')
                    <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label for="stock" class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Jumlah Stok Awal</label>
                    <input type="number" name="stock" id="stock" value="{{ old('stock', 0) }}" required min="0" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition text-sm" placeholder="Contoh: 10">
                    @error('stock')
                        <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price') }}" required min="0" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition text-sm" placeholder="Contoh: 350000">
                    @error('price')
                        <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                    @enderror
                </div>
