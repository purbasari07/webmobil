...
<div class="space-y-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        <div>
            <h2 class="text-4xl font-extrabold text-slate-800 tracking-tight">Daftar Layanan Servis</h2>
            <p class="text-slate-500 text-lg mt-2 font-medium">Daftar master jasa perawatan dan perbaikan kendaraan customer.</p>
        </div>
        <a href="{{ route('admin.services.create') }}" class="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white text-base font-bold rounded-2xl transition shadow-lg shadow-indigo-600/20 flex items-center justify-center gap-3 self-start sm:self-auto">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-slate-500 text-sm font-bold uppercase tracking-widest border-b border-slate-200">
                        <th class="px-8 py-6">Nama Layanan</th>
                        <th class="px-8 py-6">Deskripsi</th>
                        <th class="px-8 py-6">Estimasi Waktu</th>
                        <th class="px-8 py-6">Harga Dasar</th>
                ...
                <tbody class="divide-y divide-slate-100 text-base text-slate-700">
                    @forelse($services as $service)
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="px-8 py-6 text-xl font-extrabold text-slate-900">{{ $service->service_name }}</td>
                            <td class="px-8 py-6 text-slate-500 max-w-sm font-medium">{{ $service->description ?? '-' }}</td>
                            <td class="px-8 py-6">
                                <span class="inline-flex items-center px-4 py-1.5 rounded-full text-sm font-bold bg-slate-100 text-slate-800 border border-slate-200">
                                    {{ $service->estimated_time }}
                                </span>
                            </td>
                            <td class="px-8 py-6 text-xl font-extrabold text-slate-900">Rp {{ number_format($service->price, 0, ',', '.') }}</td>
                            <td class="px-8 py-6 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-4">
                                    <a href="{{ route('admin.services.edit', $service) }}" class="p-3 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-xl transition" title="Edit">
                                    ...
                                    <form action="{{ route('admin.services.destroy', $service) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus layanan ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-8 py-16 text-center text-slate-500">
                                ...
                                <span class="font-bold text-lg text-slate-600">Belum ada layanan servis</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
