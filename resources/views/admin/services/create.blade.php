...
@section('title', 'Tambah Layanan Servis')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        ...
        <h2 class="text-3xl font-bold text-slate-800 tracking-tight">Tambah Layanan Servis</h2>
        <p class="text-slate-500 text-sm mt-1">Buat data master baru untuk jenis layanan bengkel.</p>
    </div>
        ...
        <form action="{{ route('admin.services.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price') }}" required min="0" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition text-sm" placeholder="Contoh: 150000">
                    @error('price')
                        <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="estimated_time" id="estimated_time" value="{{ old('estimated_time') }}" required class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition text-sm" placeholder="Contoh: 45 Menit">
